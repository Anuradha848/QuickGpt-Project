...
const Sidebar = ({isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-7 p-3 dark:bg-gradient-to-b from[#242124]/30
     to-[#000000]/30 border-[#80609F]/30 border-2 border-r[#80609F] backdrop-blur-3xl transition-all 
     duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* Search Conversation */}
      <div className='flex items-center gap-2 mt-4 border border-gray-400
      dark:border-white/20 rounded-md'>
        <img src={assets.search_icon} className='m-4 not-dark:invert' alt="" />
        <input onChange={(e) => setSearch(e.target.value)} value={search} type="text" placeholder='Search Conversions' className='text-xs 
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          ...
          roub=nded-md cursor-pointer flex justify-between group'>
                <div>
      ...
      cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-com text-sm'>
